        ...
        <template  #table-column="props">
              ...
              <template #table-row="props">
                  <span class="flex  relative w-full text-center " v-if="props.column.field == 'actions'">
                          ...
                          <button @click="editCategory(props.row.id)" class="px-1 py-0.5 mx-2 text-md bg-green-400 rounded-lg">
                            <span style="color: aliceblue;" class="material-icons-sharp">
                                edit
                            </span>
                          </button>
                          <button @click="deleteCategory(props.row.id)" class="px-1 py-0.5 mx-2 bg-red-500 text-md rounded-lg">
                            <span style="color: aliceblue;" class="material-icons-sharp">
                                delete
                            </span>
                          </button>
...
import Pagination from '../../Pagination.vue';
      export default {
          ...
          methods: {
            getCategory(page){
                ApiService.get(`admin/categories?page=${page}`).then((res) => {
